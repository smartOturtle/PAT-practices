...
unordered_map<int, string> manititudeStringMap
{
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i < num.size(); ++i)
	{
		if(num[i]!='0')
		{
			...
			if(manititudeStringMap.find(i)!=manititudeStringMap.end())
			{
				if (i == 4)hasSetWan = true;
				else if(i>4&&i<8&&!hasSetWan)
				{
					s.push_front(manititudeStringMap[4]);
					hasSetWan = true;
				}
				s.push_front(manititudeStringMap[i]);
			}
